...
int main()
{
    ...
    do
    {
        std::cout << "\nMenue:" << std::endl
				  ...
				  << "(a): Liste nach Matrikelnummer sortieren"<<std::endl
                  << "(0): Beenden" << std::endl;
        std::cin >> abfrage;
        std::cin.ignore(10, '\n');
        ...
        switch (abfrage)
        {
            ...
            case '5':
            	...
            	{
            		unsigned int matNr;
            		std::vector<Student>::iterator gesucht;
            		std::cout<<"Bitte die Matrikelnummer vom Studenten eingeben, der gelöscht werden soll: "<<std::endl;
            		std::cin>>matNr;

            		Student student(matNr, "", "", "");

            		gesucht = std::find(studentenListe.begin(), studentenListe.end(), student);

            		if(gesucht != studentenListe.end())
            		{
            			std::cout<<"Der folgende Student wurde gelöscht: "<<std::endl;
						std::cout<<*gesucht<<std::endl;
						studentenListe.erase(gesucht);
            ...
            //Studenten in der Liste nach Matrikelnummern sortieren (aufsteigend)
            case 'a':
            {
            	std::cout<<"Die Liste wurde nach aufsteigenden Matrikelnummern sortiert."<<std::endl;
